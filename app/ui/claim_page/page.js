...
export default function ClaimPage() {
  ...
  async function handleSubmit(e) {
    ...
    // Aggregate evidence hash: keccak256(concat(hashes) || metadata)
    ...
    try {
      const uploadResp = await axios.post("/api/v1/upload-evidence", form, {
        headers: { "Content-Type": "multipart/form-data" }
      });
      const cids = uploadResp.data.cids || [];
      setStatus("Submitting claim on-chain...");
      const submitResp = await axios.post("/api/v1/submit-claim", {
        policy_id: policyId,
        evidence_hash: evidenceHash,
        ipfs_cid: cids.length ? cids[0].cid : null,
        reporter
      });
      setStatus("Done: tx " + (submitResp.data.tx_hash || JSON.stringify(submitResp.data)));
    } catch (err) {
      console.error(err);
